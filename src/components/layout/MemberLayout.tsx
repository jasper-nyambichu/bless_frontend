...
const NAV = [
  ...
  { to: '/payment',      icon: CreditCard,       label: 'Make Payment' },
  { to: '/transactions', icon: History,           label: 'Transactions' },
  { to: '/profile',      icon: User,              label: 'Profile'      },
]

const getInitials = (name: string) =>
  name ? name.split(' ').map(w => w[0]).join('').toUpperCase().slice(0, 2) : 'BP'
...
export const MemberLayout = ({ children }: Props) => {
  ...
  const currentPage = NAV.find(n => n.to === location.pathname)?.label ?? 'BlessPay'
  ...
  const SidebarInner = ({ mobile = false }: { mobile?: boolean }) => {
    ...
    return (
      <div style={{
        ...
        background: 'linear-gradient(175deg,#1a3352 0%,#0f2035 60%,#0a1828 100%)',
      }}>
        {/* Logo + toggle */}
        <div style={{
    ...
    <>
      <style>{`
        ...
        *{box-sizing:border-box}
        ...
        .bpm-layout{display:flex;min-height:100vh;background:#F0F2F5}
        .bpm-sidebar{
        ...
        .bpm-hamburger{display:none!important}
        .bpm-main{flex:1;display:flex;flex-direction:column;min-width:0}
        .bpm-content{flex:1;padding:28px 24px;width:100%;max-width:1400px;margin:0 auto}
        .bpm-uname{display:block}

        @media(max-width:1023px){
          .bpm-sidebar{display:none!important}
          .bpm-spacer{display:none!important}
          .bpm-hamburger{display:flex!important}
          .bpm-drawer.open{transform:translateX(0)!important}
          .bpm-overlay.open{display:block!important}
        }
        @media(max-width:600px){
          .bpm-content{padding:16px 12px}
          .bpm-topbar{padding:0 12px}
          .bpm-uname{display:none!important}
        }
      `}</style>

      <div className="bpm-layout">
        {/* Desktop sidebar */}
        <div className="bpm-sidebar" style={{ width: sidebarW }}>
          <SidebarInner />
        </div>
        <div className="bpm-spacer" style={{ width: sidebarW }} />

        {/* Mobile */}
        <div className={`bpm-overlay${mobileOpen ? ' open' : ''}`} onClick={() => setMobileOpen(false)} />
        <div className={`bpm-drawer${mobileOpen ? ' open' : ''}`}>
          <SidebarInner mobile />
        </div>
          ...
          <header className="bpm-topbar">
            ...
            <div
              ...
              style={{
                ...
                borderRadius: 40, padding: '4px 14px 4px 4px', cursor: 'pointer',
            ...
            >
              ...
              <span className="bpm-uname" style={{
                fontFamily: '"DM Sans",sans-serif', fontSize: 13, fontWeight: 500, color: '#374151',
              }}>{user?.username}</span>
            </div>
          </header>

          <main className="bpm-content">{children}</main>
        </div>
      </div>
    </>
  )
